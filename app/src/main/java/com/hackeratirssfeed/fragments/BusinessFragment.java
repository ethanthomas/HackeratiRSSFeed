...
public class BusinessFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        noInternet = (LinearLayout) v.findViewById(R.id.noInternet);
        ...
        public void onBindViewHolder(final ViewHolder holder, final int position) {
            ...
            holder.view.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if ((holder.view.getResources().getConfiguration().screenLayout
                            & Configuration.SCREENLAYOUT_SIZE_MASK)
                            >= Configuration.SCREENLAYOUT_SIZE_LARGE) {

                        MainActivity.loadArticle(position, "Business");

                    } else {
                        Intent intent = new Intent(v.getContext(), ArticleDetailActivity.class);
                        intent.putExtra("articleType", "business");
    ...
    private class GetFeed extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            try {
                URL url = new URL("http://www.wired.com/category/business/feed/");
        ...
        protected void onPostExecute(String result) {
            Article.BusinessArticles.clear();
            if (titles.size() == links.size() && links.size() == images.size()
                ...
                for (int i = 0; i < titles.size(); i++)
                    if (sharedPreferences.contains(titles.get(i)))
                        Article.BusinessArticles.add(new Article(titles.get(i), links.get(i), images.get(i), dates.get(i), authors.get(i), contents.get(i), true));
                    else
                        Article.BusinessArticles.add(new Article(titles.get(i), links.get(i), images.get(i), dates.get(i), authors.get(i), contents.get(i), false));
            else
            ...
            if ((getResources().getConfiguration().screenLayout
                    & Configuration.SCREENLAYOUT_SIZE_MASK)
                    >= Configuration.SCREENLAYOUT_SIZE_LARGE)
                MainActivity.loadInitialArticle(0, "Business");
            swipeRefreshLayout.setRefreshing(false);
            progressWheel.stopSpinning();
        }
    }
