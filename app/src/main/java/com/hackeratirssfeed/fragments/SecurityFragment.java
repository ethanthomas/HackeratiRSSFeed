...
public class SecurityFragment extends Fragment {
            ...
            holder.view.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if ((holder.view.getResources().getConfiguration().screenLayout
                        ...
                        MainActivity.loadArticle(position, "Security");

                    } else {
                        Intent intent = new Intent(v.getContext(), ArticleDetailActivity.class);
                        intent.putExtra("articleType", "security");
    ...
    private class GetFeed extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            try {
                URL url = new URL("http://www.wired.com/category/threatlevel/feed/");
        ...
        protected void onPostExecute(String result) {
            Article.SecurityArticles.clear();
            if (titles.size() == links.size() && links.size() == images.size()
                ...
                for (int i = 0; i < titles.size(); i++)
                    if (sharedPreferences.contains(titles.get(i)))
                        Article.SecurityArticles.add(new Article(titles.get(i), links.get(i), images.get(i), dates.get(i), authors.get(i), contents.get(i), true));
                    else
                        Article.SecurityArticles.add(new Article(titles.get(i), links.get(i), images.get(i), dates.get(i), authors.get(i), contents.get(i), false));
            else
